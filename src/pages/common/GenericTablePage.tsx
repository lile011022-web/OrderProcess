...
export function GenericTablePage({ title, desc, variant = "tasks" }: { title: string; desc?: string; variant?: Variant }) {
  ...
  return (
    <div>
      ...
      {variant === "packages" ? <PackageRows /> : variant === "buyers" ? <BuyerRows /> : <TaskRows />}
    </div>
  );
}
...
function TaskRows() {
  return <DataTable
    ...
    columns={[
      ...
      { key: "actions", title: "操作", render: () => <div className="flex gap-2"><button className="ghost-btn">查看</button><button className="ghost-btn">编辑</button></div> },
    ]}
  />;
}
...
function BuyerRows() {
  return <DataTable
    ...
    columns={[
      ...
      { key: "actions", title: "操作", render: () => <div className="flex gap-2"><button className="ghost-btn">查看</button><button className="ghost-btn">处理</button></div> },
    ]}
  />;
}
