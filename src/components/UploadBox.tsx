  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => inputRef.current?.click()} className="flex h-32 w-full items-center justify-center overflow-hidden rounded-3xl border border-dashed border-slate-300 bg-white/70 text-slate-500">
        {preview ? <img src={preview} className="h-full w-full object-cover" alt={label} /> : fileName ? <span className="flex flex-col items-center gap-2 px-3 text-center text-sm font-bold"><FileCheck2 size={24} />{fileName}</span> : <span className="flex flex-col items-center gap-2 text-sm font-bold"><ImagePlus size={24} />上传图片/PDF</span>}
      </button>
      <input
        ref={inputRef}
        type="file"
