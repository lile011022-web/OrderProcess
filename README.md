...
这是面向球星卡采购、接单、包裹入库、费用核算、付款确认、异常处理和对账管理的业务系统。v1.3.5 已加入 SQLite 持久化后端、token 登录、角色校验、审计日志、凭证上传、登录页改密、生产清除测试数据、基础安全加固、最新地址自动保留、仓库已有货物导入、多订单合包和多包裹回填，以及采购任务、接单、买手回填、付款、仓库确认、异常处理、客户资料和报表导出的真实写入闭环。
...
- 买手回填支持多个平台订单号合并到同一个运单包裹；如拆成多个物流包裹，可添加多个包裹分组。
- 采购截图、订单截图和运单截图支持点击上传，也支持复制图片后粘贴上传。
- 客户独立页面：我的采购任务、我的商品资料、我的仓库地址。
- 仓库“已收货包裹”可下载 Excel 兼容 CSV 模板，选择客户后导入当前已有货物并附带照片。
- 客户“我的包裹”可查看仓库分配给自己的货物、运单、入库成本和包裹照片。
...
- v1.3.4 起，仓库可批量导入已有货物并绑定客户，客户只能查看归属于自己的包裹和照片。
- v1.3.5 起，买手回填会清晰表达平台订单号与物流包裹的关系，并支持一条回填生成多个包裹。
...
- 使用仓库进入“已收货包裹”，下载模板并导入一批货物，确认列表出现客户、导入批次和照片数量。
- 使用客户查看我的包裹、我的商品资料和我的仓库地址，确认只显示客户自己的数据。
